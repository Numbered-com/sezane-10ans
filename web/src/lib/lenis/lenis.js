 ...
 */
	constructor ({
		...
		this.targetScroll =
		this.scroll =
		this.lastScroll = this.wrapperNode[this.scrollProperty]
	...
	raf (now) {
		...
		this.scroll += (this.targetScroll - this.scroll) * this.easing
	...
	setScroll (value) {
		const scroll = this.infinite ? clampedModulo(value, this.limit) : value
	...
	notify () {
		const scroll = this.infinite
			? clampedModulo(this.scroll, this.limit)
			: this.scroll
		const direction = this.velocity === 0 ? 0 : this.velocity > 0 ? 1 : -1
	...
	scrollTo (
		...
		if (typeof target === 'number') {
			...
			if (this.wrapperNode !== window) {
				...
				wrapperOffset = this.direction === 'horizontal' ? wrapperRect.left : wrapperRect.top
			}

			const rect = node.getBoundingClientRect()

			value = (this.direction === 'horizontal' ? rect.left : rect.top) + this.scroll - wrapperOffset
		}
